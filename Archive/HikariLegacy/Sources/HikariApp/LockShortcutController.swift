...
final class LockShortcutController {
    ...
    func start() -> Bool {
        ...
        // before a session tap receives it. Prefer the HID stage so Hikari
    ...
    func handle(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        let isHikariShortcut = event.getIntegerValueField(.keyboardEventKeycode) == 12
            && flags.contains(.maskControl)
            && flags.contains(.maskCommand)
            && !flags.contains(.maskAlternate)
            && !flags.contains(.maskShift)
        guard isHikariShortcut else {
            return Unmanaged.passUnretained(event)
        }

        if type == .keyDown,
           event.getIntegerValueField(.keyboardEventAutorepeat) == 0 {
            lockShortcutLogger.notice("Hikari Lock shortcut received")
