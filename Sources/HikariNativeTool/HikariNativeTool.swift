    ...
    static func main() {
        do {
            ...
            let supportRoot = userHome
                ...
                .appendingPathComponent("Hikari", isDirectory: true)
            ...
            let systemEnvironment: NativeLockSystemEnvironment
            switch arguments.operation {
            case .apply:
                systemEnvironment = .live
            case .restore:
                systemEnvironment = .live(
                    systemSupportRootURL:
                        NativeLockPaths.systemSupportRootForRestore(
                            transactionID: arguments.transactionID
                        )
                )
            }
            let manager = NativeLockSystemTransactionManager(
