...
final class AppModel: ObservableObject {
    ...
    @Published private(set) var settings: HikariSettings
    ...
    init() {
        do {
            ...
            let predecessorURLs = NativeStorageMigration.predecessorRootURLs(
                in: canonicalURL.deletingLastPathComponent()
            )
            if !predecessorURLs.isEmpty {
                do {
                    migrationReport = try NativeStorageMigration.migrate(
                        canonicalRootURL: canonicalURL,
                        sourceRootURLs: predecessorURLs,
                        userID: UInt32(getuid())
                    )
                } catch {
                    // Keep the predecessor directory untouched on failure.
                    // Hikari can still open its canonical library, but updates
                    // stay blocked until the migration problem is resolved.
    ...
    var menuBarHeartbeatImage: NSImage? {
        guard settings.menuBarIconStyle == .hikari else { return nil }
    ...
    var shouldPulseMenuBarSparkle: Bool {
        settings.menuBarIconStyle == .hikari && isPlaying
    }
        ...
        updateTask = Task { [weak self] in
            do {
                ...
                guard let latestVersion = release.version,
                      let currentVersion = HikariVersion(self.currentAppVersion) else {
