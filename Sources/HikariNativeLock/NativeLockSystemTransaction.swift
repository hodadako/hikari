...
public struct NativeLockSystemEnvironment: Sendable {
    ...
    public static var live: NativeLockSystemEnvironment {
        live(systemSupportRootURL: NativeLockPaths.systemSupportRoot)
    }

    public static func live(
        systemSupportRootURL: URL
    ) -> NativeLockSystemEnvironment {
        let idleAssetsRoot = NativeLockPaths.idleAssetsRoot
        return NativeLockSystemEnvironment(
            systemSupportRootURL: systemSupportRootURL,
...
public final class NativeLockSystemTransactionManager: @unchecked Sendable {
    ...
    public static let predecessorCategoryID =
        "4C554D49-4E41-4000-8000-000000000001"

    private static let managedCategoryIDs: Set<String> = [
        categoryID,
    ...
    public func apply(
        ...
        guard try !manifestContainsHikariEntries(
    ...
    public func restore(
        ...
        if let appliedManifestSHA256 = journal.appliedManifestSHA256,
           currentHash == appliedManifestSHA256 {
            ...
            restoredManifest = try manifestByRemovingHikariEntries(
                assetID: request.assetID,
    ...
    private func manifestByAdding(
        ...
        assets.removeAll { asset in
            asset["id"] as? String == request.assetID.uuidString
                || (asset["categories"] as? [String])?
                    .contains(where: Self.managedCategoryIDs.contains) == true
        }
        categories.removeAll { category in
            guard let identifier = category["id"] as? String else {
                return false
            }
            return Self.managedCategoryIDs.contains(identifier)
        }
    ...
    private func manifestByRemovingHikariEntries(
        assetID: UUID,
        from data: Data
    ) throws -> Data {
        guard var root = try JSONSerialization.jsonObject(with: data) as? [String: Any],
    ...
    private func manifestContainsHikariEntries(
        assetID: UUID,
        in data: Data
    ) throws -> Bool {
        guard let root = try JSONSerialization.jsonObject(with: data) as? [String: Any],
        ...
        return assets.contains {
            ...
            guard let identifier = $0["id"] as? String else {
                return false
            }
            return Self.managedCategoryIDs.contains(identifier)
        }
    ...
    private func rollbackAppliedManifest(
        ...
        if NativeLockDigest.sha256(of: current) == appliedHash {
            ...
            rollbackData = try manifestByRemovingHikariEntries(
                assetID: assetID,
                from: current
            )
        }
