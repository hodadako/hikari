...
public enum NativeLockPaths {
    ...
    public static let systemSupportRoot = URL(
        fileURLWithPath: "/Library/Application Support/com.hodadako.HikariNative",
        isDirectory: true
    )
    public static var predecessorSystemSupportRoot: URL {
        let predecessorProductComponent = String(
            decoding: [76, 117, 109, 105, 110, 97],
            as: UTF8.self
        )
        return URL(
            ...
            "com.hodadako.\(predecessorProductComponent)Native",
            isDirectory: true
        )
    }

    public static func systemSupportRootForRestore(
        transactionID: UUID,
        currentRootURL: URL = systemSupportRoot,
        predecessorRootURL: URL = predecessorSystemSupportRoot,
        fileManager: FileManager = .default
    ) -> URL {
        let predecessorJournalURL = predecessorRootURL
        ...
        return fileManager.fileExists(atPath: predecessorJournalURL.path)
            ? predecessorRootURL
            : currentRootURL
    }
...
public final class NativeLockUserTransactionStore: @unchecked Sendable {
    ...
    /// touching the privileged system asset. Any non-Hikari choice that will
