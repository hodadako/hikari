...
final class StoreTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        temporaryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("HikariTests-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    func testLegacySettingsDefaultToHikariIcon() throws {
        let container = try SharedContainer(rootURL: temporaryURL)
        let legacyJSON = """
        {
          "isMuted": true,
        ...
        XCTAssertEqual(settings.appIconStyle, .hikari)
        XCTAssertNil(settings.customAppIconRelativePath)
        XCTAssertEqual(settings.menuBarIconStyle, .hikari)
    ...
    func testLegacyPresetIconsMigrateToHikariIcons() throws {
        let container = try SharedContainer(rootURL: temporaryURL)
        let legacyJSON = """
        {
          "appIconStyle": "pink",
    ...
    func testCorruptSettingsFallBackToDefaults() throws {
        ...
        XCTAssertEqual(SettingsStore(container: container).load(), HikariSettings())
    }
