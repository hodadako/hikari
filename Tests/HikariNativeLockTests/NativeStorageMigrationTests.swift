...
final class NativeStorageMigrationTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        canonicalRootURL = rootURL.appendingPathComponent("Hikari")
        legacyRootURL = rootURL.appendingPathComponent(
            NativeStorageMigration.predecessorDirectoryNames.last!,
            isDirectory: true
        )
    ...
    func testFindsAllPredecessorRootsInPriorityOrder() throws {
        let predecessorCanonicalRoot = rootURL.appendingPathComponent(
            NativeStorageMigration.predecessorDirectoryNames.first!,
            isDirectory: true
        )
        try FileManager.default.createDirectory(
            at: predecessorCanonicalRoot,
            withIntermediateDirectories: true
        )

        XCTAssertEqual(
            NativeStorageMigration.predecessorRootURLs(in: rootURL),
            [predecessorCanonicalRoot, legacyRootURL]
        )
    }

    func testMergesAllPredecessorRoots() throws {
        let predecessorCanonicalRoot = rootURL.appendingPathComponent(
            NativeStorageMigration.predecessorDirectoryNames.first!,
            isDirectory: true
        )
        ...
        let secondID = UUID()
        try FileManager.default.createDirectory(
            at: predecessorCanonicalRoot.appendingPathComponent(
                "Media",
                isDirectory: true
            ),
            withIntermediateDirectories: true
        ...
        try Data("first".utf8).write(
            to: predecessorCanonicalRoot.appendingPathComponent("Media/first.mov")
        )
        try writeContents(
            [
                makeContent(
                    id: firstID,
                    relativePath: "Media/first.mov",
                    thumbnailRelativePath: nil
                )
            ],
            to: predecessorCanonicalRoot.appendingPathComponent("contents.json")
        )
        try Data("second".utf8).write(
            to: legacyRootURL.appendingPathComponent("Media/second.mov")
        )
        try writeContents(
            [
                makeContent(
                    id: secondID,
                    relativePath: "Media/second.mov",
                    thumbnailRelativePath: nil
                )
            ],
        ...
        let sourceRoots = NativeStorageMigration.predecessorRootURLs(in: rootURL)
        let report = try NativeStorageMigration.migrate(
            canonicalRootURL: canonicalRootURL,
            sourceRootURLs: sourceRoots,
            userID: UInt32(getuid())
        )

        XCTAssertEqual(report.migratedContentCount, 2)
        XCTAssertEqual(report.archivedSourceRootURLs.count, 2)
        XCTAssertFalse(
            FileManager.default.fileExists(atPath: predecessorCanonicalRoot.path)
        )
        ...
        XCTAssertEqual(
            Set(merged.compactMap { $0["id"] as? String }),
            Set([firstID.uuidString, secondID.uuidString])
        )
    }

    func testReadsVersionOneSingleArchiveMarker() throws {
        let archivedRoot = rootURL.appendingPathComponent(
        ...
        let marker: [String: Any] = [
            "schemaVersion": 1,
            "didMigrate": true,
            "migratedContentCount": 3,
            "migratedTransactionCount": 1,
            "requiresRestore": true,
            "archivedLegacyRootPath": archivedRoot.path
        ]
        try JSONSerialization.data(
            withJSONObject: marker,
            options: [.prettyPrinted, .sortedKeys]
        ).write(
            to: canonicalRootURL.appendingPathComponent(
                NativeStorageMigration.migrationMarkerFilename
            )
        )
        ...
        XCTAssertEqual(report.migratedContentCount, 3)
        XCTAssertEqual(report.migratedTransactionCount, 1)
        XCTAssertTrue(report.requiresRestore)
        XCTAssertEqual(report.archivedSourceRootURLs, [archivedRoot])
    }
    ...
    func testMergesLegacyLibraryAndArchivesOriginalRoot() throws {
        ...
        XCTAssertEqual(report.archivedSourceRootURLs.count, 1)
        XCTAssertFalse(FileManager.default.fileExists(atPath: legacyRootURL.path))
        XCTAssertTrue(
            FileManager.default.fileExists(
                atPath: report.archivedSourceRootURLs[0].path
            )
        )
        XCTAssertEqual(
